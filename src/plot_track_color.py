...
#    print( rain_l )
    ...
    for i, mem in enumerate( mem_l[::-1] ):
        ...
        m_l[0].plot( xs_, ys_, color=cc, linewidth=lw, alpha=0.5 )  
        m_l[0].plot( xs_[cit], ys_[cit], color=cc2, marker="o",
               markersize=ms, label=lab_c, zorder=5, alpha=0.5 )  
    ...
    opath = "png/track"
    ofig = "1p_mark_track_s{0:}_dlon{1:}_ng{2:0=3}_{3:}".format( stime.strftime('%m%d'), dlon, ng, ctime.strftime('%m%d%H'),  ) 
