...
from tools_BAIU import get_lonlat, prep_proj_multi, get_mslp, get_the
...
def main( vtime=datetime(2018, 7, 1, 0 ), hpa=500 ):
    ...
    var_ =  get_the( INFO, stime=vtime, vtime=vtime, m=0, hpa=hpa ) 
    ...
    tit = r'Analysis $\theta_e${0:} (K), valid: {1:}'.format( hpa, vtime.strftime('%HUTC %m/%d') )
    ofig = "THE{0:}_v{1:}".format( hpa, vtime.strftime('%H%m%d') )
