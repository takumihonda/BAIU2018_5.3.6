...
from tools_BAIU import get_lonlat, get_arain, get_pw, prep_proj_multi, draw_rec, get_gph, get_prsvar, get_the, get_the_grad, get_rh, get_mslp

quick = True   
#quick = False


def main( nstime=2, stime_l=[],
    ...
    lon2d, lat2d = get_lonlat( INFO, stime=stime_l[0] )




    pw_l = np.zeros( 50*nstime )
    rain_l = np.zeros( 50*nstime )


    # get reference
    mm = 0
    for sidx in range( nstime):
        stime = stime_l[sidx]
        for m in range( 50 ):
            rain_ = get_arain( INFO, stime=stime, vtime=vtime_ref, adt=adt, m=m+1 )
            rain_l[mm] = np.mean( rain_[ (lon2d >= slon ) & (lon2d <= elon) & (lat2d >= slat) & (lat2d <= elat) ] )

            pw_ = get_pw( INFO, stime=stime, vtime=vtime_ref, m=m+1, )
            pw_l[mm] = np.mean( pw_[ (lon2d >= slon ) & (lon2d <= elon) & (lat2d >= slat) & (lat2d <= elat) ] )
...
#    print( "spread",np.std(rain_l, ddof=1) )
    ...
    for vtime in vtime_l:
       ...
       for mm in range( 50*nstime ):
           sidx = mm // 50
           m = mm - sidx * 50
           stime = stime_l[sidx] 
           print( m, mm, sidx, stime)

           if tvar == "PW":
              var_ = get_pw( INFO, stime=stime, vtime=vtime, m=m+1, )
           elif tvar == "MSLP":
              var_ = get_mslp( INFO, stime=stime, vtime=vtime, m=m+1, )
           elif tvar == "Z":
       ...
       ecor = ecor / np.std( ref_l, ddof=1 ) / np.std( evar, ddof=1, axis=0 ) / 50.0
       ...
       if tvar == "PW" or tvar == "RAIN" or tvar == "MSLP":
          ttvar = tvar
       else:
          ttvar = tvar + str( hpa )
       ...
       ofig = "ecor_r{0:}_t{1:}_v{2:}_vr{3:}".format( rrvar, ttvar, vtime.strftime('%H%m%d'), vtime_ref.strftime('%H%m%d') ) 
...
stime_l = [ datetime( 2018, 7, 1, 0), datetime( 2018, 7, 2, 0), datetime( 2018, 7, 3, 0 ) ]
nstime = 2

for tvar in tvar_l:
    
    for hpa in hpa_l:
       main( nstime=nstime, stime_l=stime_l, vtime_l=vtime_l,  
             rvar=rvar, tvar=tvar, hpa=hpa )
       if tvar == "PW" or tvar == "RAIN" or tvar == "UDIF":
          break
