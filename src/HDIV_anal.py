...
from tools_BAIU import get_lonlat, prep_proj_multi, get_mslp, get_hdiv_curl
...
def main( vtime=datetime(2018, 7, 1, 0 ), hpa=500 ):
    ...
    var_, _ =  get_hdiv_curl( INFO, stime=vtime, vtime=vtime, m=0, hpa=hpa )

    print( np.nanmax( var_))    
    print( np.nanmin( var_))    
...
#       extend = 'both'
    ...
    tit = r'Analysis HDIV{0:} (10$^4$s$^{{-1}}$), valid: {1:}'.format( hpa, vtime.strftime('%HUTC %m/%d') )
    ofig = "hdiv{0:}_v{1:}".format( hpa, vtime.strftime('%H%m%d') )
