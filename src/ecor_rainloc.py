...
from tools_BAIU import get_lonlat, get_arain, get_pw, prep_proj_multi, draw_rec, get_gph, get_prsvar, get_the, get_the_grad, get_rh, get_mslp, get_mf_grad, get_q1, read_etrack
...
#    print( len(mem_l) )
    ...
    for m in range( 50 ):
        ...
        rain_[ np.abs( lon2d - clon ) >  dlon ] = np.nan
        cy, cx = np.unravel_index( np.nanargmax(rain_), rain_.shape) 
        rlat_l[m] = lat2d[cy,cx]

    rlat_l -= np.mean( rlat_l )
