...
from tools_BAIU import get_lonlat, prep_proj_multi, get_mslp, get_ki
...
def main( vtime=datetime(2018, 7, 1, 0 ) ):
    ...
    var = get_ki( INFO, stime=vtime, vtime=vtime, m=0 )
    ...
    tit = r'Analysis ensemble mean K index ($^\circ$C), valid: {0:}'.format( vtime.strftime('%HUTC %m/%d') )
