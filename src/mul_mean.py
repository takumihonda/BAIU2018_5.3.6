...
from tools_BAIU import get_lonlat, prep_proj_multi, get_arain, get_mslp, get_pw, get_gph, get_prsvar, get_the, get_the_grad, get_rh, get_ki, get_hdiv_curl, get_the_grad, get_q1, def_cmap
    ...
    for i, ax in enumerate( ax_l):
       ...
       for m in range( 0, mmax ):
           ...
           if nvar == "PW":
              ...
              var_ =  get_prsvar( INFO, nvar="Tprs", stime=stime, vtime=vtime, m=m+1, hpa=hpa )
              var_[ var_ < 0.0 ] = np.nan
...
#          ax.clabel( CONT0, fontsize=8, fmt='%.0f' )
    ...
    if nvar == "Z" or nvar == "MSLP":
       tit = '{0:}, valid: {1:}'.format( tnvar, vtime.strftime('%HUTC %m/%d') )
    else:
       tit = '{0:} & {1:}, valid: {2:}'.format( tnvar, tnvar2, vtime.strftime('%HUTC %m/%d') )

    fig.suptitle( tit, fontsize=12 )
        
    opath = "png/mean_multi"
