...
def main( vtime=datetime(2018, 7, 1, 0 ), hpa=500 ):
    ...
    gph_ = get_gph( INFO, stime=vtime, vtime=vtime, m=0 )
    var_ =  get_prsvar( INFO, nvar="Uprs", stime=vtime, vtime=vtime, m=0, hpa=hpa ) 
    ...
    clevs = np.arange( 800, 12000, 20 )
    CONT = ax1.contour( x2d, y2d, gph_, colors='k',#'gainsboro',
...
#    ax1.clabel( CONT, fontsize=8, fmt='%.0f' )
    ...
    tit = r'Analysis U{0:} (m s$^{{-1}}$), valid: {1:}'.format( hpa, vtime.strftime('%HUTC %m/%d') )
    ofig = "U{0:}_v{1:}".format( hpa, vtime.strftime('%H%m%d') )
