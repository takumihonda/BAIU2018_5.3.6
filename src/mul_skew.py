...
def main( stime_l=[],
    ...
    if nvar == "PW":
       ...
       levs = np.arange( -3.0, 3.25, 0.25 )
       unit = r'(g/m$^3$)'
       if hpa < 700:    
          levs = np.arange( -2.0, 2.1, 0.1 )
    elif nvar == "RH":
...
#    mmax = 5 # DEBUG
    ...
    tit = '{0:} skewness & MSLP, valid: {1:}'.format( tnvar, vtime.strftime('%HUTC %m/%d') )

    fig.suptitle( tit, fontsize=12 )
        
    opath = "png/skew_multi"
    ofig = "skew_{0:}_v{1:}".format( tnvar_, vtime.strftime('%H%m%d') ) 
