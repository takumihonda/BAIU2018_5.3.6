...
def main( stime=datetime( 2018, 6, 30, 0), vtime_ref=datetime( 2018, 7, 6, 0 ),
          vtime=datetime( 2018, 7, 5, 0 ), vtime2=datetime( 2018, 7, 5, 0 ),
          nvar="RAIN", nvar2="MSLP", 
...
#    lc = 'w'
    ...
    for m in range( len( mem_l ) ):
        ...
        if m == 0:
           ...
           mvar_2 = get_var( INFO, nvar=nvar, stime=stime, vtime=vtime2, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
           mvar2_2 = get_var( INFO, nvar=nvar2, stime=stime, vtime=vtime2, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
        else:
           mvar_ += get_var( INFO, nvar=nvar, stime=stime, vtime=vtime, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
           mvar2_ += get_var( INFO, nvar=nvar2, stime=stime, vtime=vtime, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
    ...
    mvar_2 = mvar_2 / len( mem_l )
    mvar2_2 = mvar2_2 / len( mem_l )
...
#       m = pmem_l[i]
       ...
       for j, m in enumerate( pmem_l[i] ):
           ...
           if j == 0:
              ...
              var_2 = get_var( INFO, nvar=nvar, stime=stime, vtime=vtime2, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
              var2_2 = get_var( INFO, nvar=nvar2, stime=stime, vtime=vtime2, m=m+1, adt=timedelta( hours=0 ), hpa=hpa2 )
           else:
              var_ += get_var( INFO, nvar=nvar, stime=stime, vtime=vtime, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
              var2_ += get_var( INFO, nvar=nvar2, stime=stime, vtime=vtime, m=m+1, adt=timedelta( hours=0 ), hpa=hpa2 )
...
#       ax.clabel( CONT, fontsize=6, fmt='%.0f' )
       ...
       if i == 0 or i == 2:
          ...
          gtit = "{0:} & {1:}, init:{2:}, valid:{3:}".format( nvar_, nvar2_, stime.strftime('%HUTC %m/%d'), vtime_.strftime('%HUTC %m/%d') )
          ax.text( 0.5, 1.02, gtit,
    ...
    if nvar2 is not None:
       ofig = "4p_difm_{0:}_{1:}_{2:}_adt{3:0=3}".format( nvar_, nvar2_, vtime.strftime('v%m%d%H'), adt_h, )
...
#    fig.suptitle( gtit, fontsize=12)
    ...
    if not quick:
       opath = "png/4p_cmem" + stime.strftime('_s%H%m%d_cmem') + str( cmem ).zfill(2)
