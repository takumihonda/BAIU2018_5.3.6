...
def main( vtime=datetime(2018, 7, 1, 0 ), hpa=500 ):
    ...
    _, var_ =  get_hdiv_curl( INFO, stime=vtime, vtime=vtime, m=0, hpa=hpa )
...
#       extend = 'both'
    ...
    tit = r'Analysis $\zeta${0:} (10$^4$s$^{{-1}}$), valid: {1:}'.format( hpa, vtime.strftime('%HUTC %m/%d') )
    ofig = "curl{0:}_v{1:}".format( hpa, vtime.strftime('%H%m%d') )
