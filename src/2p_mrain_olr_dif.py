...
def main( stime=datetime( 2018, 6, 30, 0), 
    ...
    stime2 = stime - timedelta( hours=6 )
    ...
    levs = np.arange( -80, 85, 5 )
    cmap3 = plt.cm.get_cmap("RdBu_r")
    levs3 = np.arange( -80, 85, 5 )
    levs3 = np.arange( -40, 45, 5 )
    ...
    for m in range( 50 ):
        if m == 0:
           ...
           var_2 = get_var( INFO, nvar=nvar, stime=stime2, vtime=vtime, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
           ...
           var3_2 = get_var( INFO, nvar="OLR", stime=stime2, vtime=ovtime, m=m+1, adt=timedelta( hours=0 ), hpa=hpa2 )
        else:
           var_ += get_var( INFO, nvar=nvar, stime=stime, vtime=vtime, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
           var_2 += get_var( INFO, nvar=nvar, stime=stime2, vtime=vtime, m=m+1, adt=timedelta( hours=dth_ ), hpa=hpa )
           var2_ += get_var( INFO, nvar=nvar2, stime=stime, vtime=ovtime, m=m+1, adt=timedelta( hours=0 ), hpa=hpa2 )
