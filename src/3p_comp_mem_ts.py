...
def main( stime=datetime( 2018, 6, 30, 0), 
    ...
    for m in mem_l[0:mem_max]:
        ...
        if nvar == "MSLP":
           ...
           var_b += get_pw( INFO, stime=stime, vtime=vtime, m=m )
        else:
           var_b += get_prsvar( INFO, nvar=nvar, stime=stime, vtime=vtime, m=m, hpa=hpa )

        mslp_ = get_mslp( INFO, stime=stime, vtime=vtime, m=m )
    ...
    for m in mem_l[len(mem_l)-mem_max:len(mem_l):1]:
        ...
        if nvar == "MSLP":
           ...
           var_w += get_pw( INFO, stime=stime, vtime=vtime, m=m )
        else:
           var_w += get_prsvar( INFO, nvar=nvar, stime=stime, vtime=vtime, m=m, hpa=hpa )
    ...
    for i, ax in enumerate( ax_l ):
        ...
        if i == 2:
           SHADE = ax.contourf( x2d, y2d, var_l[i], 
                                cmap=cmap, levels=dif_clevs, extend='both' )
