...
def main( slon=130.0, elon=137.5,
    ...
    ax.plot( stime_l, std_l, linewidth=lw, color='k' )
    ax.plot( stime_l, mean_l, linewidth=lw, color='r' )
    ...
    ax.vlines( x=stime_l, ymin=ymin, ymax=ymax, linestyle='dashed',
               lw=0.5, color='gray' )

    opath = "png/sprd_ts"
    ofig = "1p_sprd_{0:}_lon{1:}_{2:}_lat{3:}_{4:}_hpa{5:}_adt{6:0=3}".format( nvar_, slon, elon, slat, elat, hpa, dth ) 

    if nvar_ == "RAIN":
       tit = "Forecast ensemble spread of\n{0:}-h accumulated precipitation amount".format( dth ) #"{0:} forecast spread at {1:}".format( nvar_, vtime_ref.strftime('%HUTC %m/%d') ) 

       note = "Period: {0:}\n-{1:}".format( ( vtime_ref - timedelta(hours=dth) ).strftime('%HUTC %m/%d'),
                 vtime_ref.strftime('%HUTC %m/%d'), )
       ax.text( 0.99, 0.99, note,
