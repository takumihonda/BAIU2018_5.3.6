...
from tools_BAIU import get_lonlat, prep_proj_multi, get_mslp, get_rh
...
def main( vtime=datetime(2018, 7, 1, 0 ), hpa=500 ):
    ...
    var_ =  get_rh( INFO, stime=vtime, vtime=vtime, m=0, hpa=hpa ) * 1.e2
    ...
    tit = r'Analysis RH{0:} (%), valid: {1:}'.format( hpa, vtime.strftime('%HUTC %m/%d') )
    ofig = "RH{0:}_v{1:}".format( hpa, vtime.strftime('%H%m%d') )
