...
from tools_BAIU import get_lonlat, prep_proj_multi, get_mslp, get_the_grad
...
def main( vtime=datetime(2018, 7, 1, 0 ), hpa=500 ):
    ...
    vx_, vy_ = get_the_grad( INFO, stime=vtime, vtime=vtime, m=0, hpa=hpa ) 
    ...
    tit = r'Analysis |$\nabla \theta_e${0:}| (10$^{{-1}}$K/km), valid: {1:}'.format( hpa, vtime.strftime('%HUTC %m/%d') )
    ofig = "THEG{0:}_v{1:}".format( hpa, vtime.strftime('%H%m%d') )
