...
def main( 
    ...
    for m in range( mmax ):
        ...
        pw_ = get_var( INFO, nvar="PW", stime=stime, vtime=ctime, m=m+1 )
        pw_[ np.abs( lon2d - clon ) >  dlon ] = np.nan
        cy, cx = np.unravel_index( np.nanargmax(pw_), pw_.shape) 
        lat_l[m] = lat2d[cy,cx]
        print( pw_[cy,cx], lat2d[cy,cx] )
 

    print( lat_l.shape, rain_l.shape )
    plt.scatter( lat_l, rain_l )
    plt.show()
...
#    blon, blat, bmslp = get_prapiroon( time=stime )
...
#    xlab = "TC longitude (deg)"
    xlab = "TC SLP valid at {0:} (hPa)".format( stime.strftime('%HUTC %m/%d'), )
...
#              ha='right', va='top', )
    ...
    opath = "png/track"
    ofig = "1p_scat_track_s{0:}_dlon{1:}_ng{2:0=3}_{3:}".format( stime.strftime('%m%d'), dlon, ng, ctime.strftime('%m%d%H'),  ) 
